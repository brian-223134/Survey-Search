...
**현재 상태 (2026-08-13): P0~P5 구현·측정 완료, 테스트 195개 통과.**
파이프라인 전 단계(dense+BM25+RRF+dedup+freshness+MMR+facet+스노우볼링+재랭킹)와
온라인 백엔드(arXiv·S2), **SurGE 정답 토픽 170개 전체 평가**까지 끝났습니다.

**측정이 말하는 결론 한 줄: `facets=True, freshness=True` 가 최선이고, 다양성(MMR)과
cross-encoder 재랭킹은 켜면 손해입니다.** 셋 다 "당연히 좋을 것"이었는데 정답 기준으로는
기준선에 미달했고, 공통 원인은 **facet 이 만든 다중 쿼리 합의(RRF) 신호를 파괴**한다는
것입니다.

**170토픽 전체 결과 (2026-08-13 03:52 완료)** — 표본을 10배로 늘려도 결론이 안 바뀝니다:

| 설정 | R@50 | R@1500 | nDCG |
|---|---|---|---|
| dense only | 15.4% | 38.7% | 0.200 |
| + BM25 | 13.4% | 41.7% | 0.177 |
| + freshness | 14.0% | 41.7% | 0.217 |
| **+ facets** | **19.0%** | **56.9%** | **0.294** |

표본이 늘려 달라진 것은 **BM25 의 nDCG 하나**입니다 — 16토픽에서는 무해(0.189→0.189)해
보였는데 170토픽에서는 **−0.023 으로 분명한 손해**입니다. R@1500 은 여전히 +3.1%p 라
꼬리 recall 을 사고 켜 두지만 공짜가 아닙니다.

**권장 설정은 `facets=True, freshness=True, lexical=False` 입니다.** BM25 는 facet 이
켜지면 하는 일이 없고(다섯 지표 모두 `|평균차| < 2×SE`) 시간만 5배 씁니다 —
`docs/bm25-in-facets.md`. facet 을 안 쓸 거면 BM25 는 켜 두세요.

**다음 할 일: P3 호스트 스모크.** SurveyForge 어댑터 배선 4건을 고쳐(커밋 `f198a58`)
선행 작업은 끝났습니다. 남은 것은 **호스트 venv 설치**(승인 필요 — 베이스라인 arm 이
그 환경에서 생성됨)와 실제 호스트를 태우는 확인 4가지이고 **전부 $0** 입니다.
절차는 `../SURVEY-SEARCH.md` §6, 경위는 `HANDOFF.md` §12.

**열린 결정 하나** — 재랭킹을 facet 내 순위 + RRF 융합으로 한 번 더 시도할지,
사전 중단 기준대로 접을지 (`HANDOFF.md` §8).
...
- **1시간 넘는 평가는 체크포인트 없이 띄우지 마세요.** `eval.surge --checkpoint <경로>`.
  체크포인트 없이 돌리다 죽어서 30토픽분을 날린 적이 있습니다
- **프로세스가 사는지 `pgrep`만으로 판단하지 마세요.** `pgrep -f "python scripts/x.py"`는
  그 문자열을 인자로 가진 **셸 래퍼까지** 잡습니다(앵커: `^[^ ]*python scripts/x\.py$`).
  살아 있어도 멈춰 있을 수 있으니 `/proc/<pid>/stat`의 CPU 시간 증가로 확인하세요
- **네트워크를 타는 코드에 벽시계 상한이 있는지 보세요.** `urlopen(timeout=)`은
  소켓 연산 1회 상한이라 keep-alive가 오면 영영 안 걸립니다 (`HANDOFF.md` §3 함정 10)

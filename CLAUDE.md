...
**현재 상태 (2026-08-13): P0~P5 구현·측정 완료, 테스트 181개 통과.**
...
**다음 판단이 필요한 것 둘**
1. 재랭킹을 facet 내 순위 + RRF 융합으로 한 번 더 시도할지, 사전 중단 기준대로 접을지
   (`HANDOFF.md` §8)
2. **facet 을 켠 상태에서 BM25 가 여전히 이득인지** — 지금 표는 설정이 누적이라
   `facets + freshness − bm25` 행이 없습니다. facet 캐시가 다 차서 LLM 0회·25분이면 됩니다

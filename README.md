...
**여기가 파이프라인에서 유일하게 네트워크를 타는 곳이고, 그래서 유일하게 무한정
서 있을 수 있는 곳입니다.** 실측 정상 지연이 40초인데, 프로바이더가 응답을 안 끝내면서
keep-alive 바이트만 흘려보내면 `urlopen(timeout=)` 이 매 읽기마다 초기화돼 **상한이
영영 발동하지 않습니다.** 그래서 벽시계 상한(`deadline_s`, 기본 120초)을 따로 겁니다.
배치로 돌릴 땐 `scripts/prewarm_facets.py` 로 캐시를 미리 채우면 본체가 네트워크를
아예 안 탑니다 (`SHARD`/`NSHARDS` 로 분할 병렬).

비용은 호출당 **$0.0006** (실측). 출력 토큰의 67%가 캐시에 안 남는 추론 토큰이라
캐시 파일 크기로 역산하면 5배 과소평가됩니다.
...
**긴 평가는 체크포인트를 켜세요.** 170토픽 × 4설정 = 검색 680회는 2시간이 넘고,
중간에 죽으면 전부 날아갑니다(실제로 겪었습니다).
...
`scripts/watch_surge_eval.sh` 는 그 위에서 죽음(60초)과 정지(25분)를 판정해 재시작합니다.
...
**표본을 10배로 늘려도 결론이 안 바뀝니다.** 16토픽 대비 최대 편차가 facet 의
R@1500 −2.2%p 이고 나머지는 1%p·0.01 이내입니다. 16토픽 표본이 운이 좋았던 게
아니라는 뜻입니다.

**표본이 늘려서 달라진 것은 BM25 의 nDCG 하나입니다.** 16토픽에서는 0.189 → 0.189
로 무해해 보였는데, 170토픽에서는 **0.200 → 0.177 로 분명히 손해**입니다. 격차가
작아서 표본이 필요했던 바로 그 항목이고, 이걸 확인하려고 170토픽을 돌렸습니다.

<details>
<summary><b>토픽 16개 결과</b> — 다양성(MMR)은 여기서만 쟀습니다 (펼치기)</summary>
...
위 ①②는 170토픽에서도 그대로입니다. ③④는 170토픽 기준으로 아래처럼 갱신됩니다.

**③ BM25 는 머리를 깎고 꼬리를 늘립니다 — 그리고 순서도 깎습니다.** R@50 −2.0%p,
R@1500 **+3.1%p**, nDCG **−0.023**. 서베이는 수백~수천 편을 모으는 작업이라 꼬리
recall 을 사고 켜 두지만, **공짜가 아닙니다.** 상위권만 쓰는 용도라면 끄는 게 낫습니다.

**⑤ 그런데 facet 을 켜면 BM25 는 아무것도 안 합니다** (2026-08-13 측정, 같은 170토픽)

위 표는 설정이 누적이라 `+facets` 행에도 BM25 가 켜져 있습니다. 그래서 BM25 를 뺀
설정을 따로 돌렸습니다.
...
격차가 0.5%p 수준이라 평균만으로는 판단할 수 없습니다. 토픽별로 짝지어 보면:
...
**넷 다 `|평균차| < 2×SE` 라 방향을 단정할 수 없습니다.** 즉 facet 이 켜진 상태에서
BM25 는 **측정 가능한 차이를 만들지 않습니다.**

그런데 이게 답입니다. 대조해 보면 분명합니다:
...
**BM25 가 주던 +3.1%p 를 facet fan-out 이 전부 흡수했습니다.** 토픽을 하위 주제
12~16개로 쪼개 표면형이 다른 쿼리를 던지면 어휘 검색이 메우던 구멍이 이미 메워지고,
...
**④ freshness 는 recall 을 안 건드리고 순서만 개선합니다.** R@1500 은 41.7% 로 같은데
nDCG 가 0.177 → 0.217 (**+0.040**). BM25 가 깎아먹은 0.023 을 되돌리고도 남습니다.
같은 논문을 더 앞에 놓는다는 뜻이고, 이게 이 프로젝트의 원래 가설이었습니다.
...
호출 전체의 상한이 아닙니다. OpenRouter(Cloudflare 뒤)는 생성이 긴 요청에 **10초마다
912바이트**의 keep-alive 를 흘려보내는데, 그때마다 읽기가 성공하니 타이머가 초기화되고
90초 상한은 **영영 발동하지 않습니다.** 재시도 횟수도 예외가 안 나니 소용이 없습니다.

진단은 `/proc/<pid>/stat` 의 CPU 시간이 20초에 0.01초만 늘고, `ss -tnp` 에는 소켓이
살아 있으며, 수신 바이트가 정확히 10초 간격으로 912씩 느는 것으로 했습니다.

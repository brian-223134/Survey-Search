...
> **아직 답 못 한 것**: 위 표의 설정은 누적이라 `facets` 행에도 BM25 가 켜져 있습니다.
> **facet fan-out 이 이미 BM25 가 주는 것을 덮고 있는지**는 따로 재야 압니다
> (`facets=True, freshness=True, lexical=False` 행이 없습니다). facet 캐시가 다 찬
> 지금은 LLM 호출 0회·약 25분이면 되는 실험입니다.

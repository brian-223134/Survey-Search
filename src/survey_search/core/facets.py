...
**파이프라인에서 LLM을 쓰는 곳은 여기 하나뿐입니다.** 결과는 디스크 캐시라 같은 토픽
재실행은 호출 0회입니다.

> 이 자리에 "그래서 실험도 결정적입니다"라고 적혀 있었는데 **사실이 아니었습니다.**
> LLM 호출이 캐시되는 것과 파이프라인이 결정적인 것은 별개입니다. 실제로는 DuckDB
> BM25 의 정렬이 실행마다 흔들려 같은 토픽 재검색이 다른 결과를 냈습니다
> (`backends/faiss_duckdb.py` 의 `lexical_search` 주석 참고). 지금은 고쳐져서
> 결정적이고, `tests/test_determinism.py` 가 그것을 붙잡고 있습니다.
